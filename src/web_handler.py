...
class webHandlerIface:
    def get_data(self):
        pass

    def find_weekend(self, month=1, year=datetime.datetime.now().year):
    ...
    def get_booking_page(
        self, city, dest_id, rooms, people, startdate, enddate, offset
    ):
        """
    ...
    def prep_data(
        ...
        parsed_html = self.get_booking_page(
...
class WeekWebHandler(webHandlerIface):
    def __init__(self):
        pass
    ...
    def get_data(
        ...
        if weekend:
            ...
            it = iter(pd.Series(self.find_weekend(month)))
            nocleg_series = pd.Series([*zip(it, it)])
        elif month != 0:
            start = (
                "{year}-{month}-01".format(
                    year=datetime.datetime.now().year, month=month
                )
                if month > curr_month
                else start_date
            )
            # end = pd.Series(pd.date_range(start=start_date, end=end_date, freq='ME'))[-1]
            # print(pd.date_range(start=start_date, end=end_date, freq='ME')[-1])
            end = end_date
        elif month == 0:
...
class WeekendWebHandler(webHandlerIface):
    def __init__(self):
        pass
    ...
    def prep_data(
        ...
        return super().prep_data(
            city, dest_id, rooms, people, start_date, end_date, timeofstay, offset
        )
